...
object SvgPathEmitter {
    ...
    /**
     * Compatibility entry point used by SvgTreeConverter. Dash state now lives
     * in SvgDashContext rather than SvgPathEmitter or SvgDashApproximator.
     */
    internal fun pushDashTransformCompensation(
        enabled: Boolean,
        scale: Float,
        approximate: Boolean
    ) {
        SvgDashContext.pushTransformCompensation(enabled, scale, approximate)
    }

    internal fun popDashTransformCompensation() {
        SvgDashContext.popTransformCompensation()
    }
