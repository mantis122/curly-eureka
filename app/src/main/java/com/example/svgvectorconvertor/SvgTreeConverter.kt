...
val currentClipPath = SvgPaintResolver.styleValue(style, "clip-path")
   ...
   val tagName = element.tagName.substringAfter(":").lowercase()
    ...
    when (tagName) {
        ...
        "rect", "circle", "ellipse", "line", "polyline", "polygon" -> {
            appendBasicShapePathWithGeometryFallback(
                output,
                element,
                tagName,
                indent,
...
private fun appendBasicShapePathWithGeometryFallback(
    output: StringBuilder,
    element: Element,
    tagName: String,
    indent: String,
    ...
    val transforms = SvgTransformParser.parseTransformList(element.getAttribute("transform"))
    val geometryTransform = SvgTransformParser.geometryFallbackTransform(transforms)
    ...
    val transformedPathData = transformedBasicShapePathData(element, tagName, geometryTransform)
    if (transformedPathData == null) {
        appendBasicShapePathCallback(
            output,
            element,
            tagName,
    ...
    val pathElement = element.ownerDocument.createElement("path")
    copyDrawableAttributesForGeometryFallback(element, pathElement)
    pathElement.setAttribute("d", transformedPathData)

    output.appendLine("${indent}<!-- converted from <$tagName>; transform baked into pathData -->")
    appendElementPathCallback(
        output,
        pathElement,
        indent,
...
private fun transformedBasicShapePathData(
    element: Element,
    tagName: String,
    ...
    return when (tagName) {
        "rect" -> {
            ...
            val width = floatAttrCallback(element, "width") ?: return null
            val height = floatAttrCallback(element, "height") ?: return null
            if (width <= 0f || height <= 0f) return null

            val rx = floatAttrCallback(element, "rx") ?: 0f
            val ry = floatAttrCallback(element, "ry") ?: 0f
            if (rx != 0f || ry != 0f) return null

            SvgTransformParser.transformRectPathData(x, y, width, height, transform)
        }
        "line" -> {
            val x1 = floatAttrCallback(element, "x1") ?: 0f
            val y1 = floatAttrCallback(element, "y1") ?: 0f
            val x2 = floatAttrCallback(element, "x2") ?: 0f
            val y2 = floatAttrCallback(element, "y2") ?: 0f
            SvgTransformParser.transformLinePathData(x1, y1, x2, y2, transform)
        }
        "polygon", "polyline" -> {
            val points = parseSvgPoints(element.getAttribute("points"))
            if (points.isEmpty()) return null
            SvgTransformParser.transformPolygonPathData(points, transform, close = tagName == "polygon")
        }
        else -> null
    }
}
...
private fun parseSvgPoints(points: String): List<SvgPoint> {
    ...
    return values
        .chunked(2)
        .mapNotNull { pair ->
            if (pair.size == 2) SvgPoint(pair[0], pair[1]) else null
        }
}

private fun copyDrawableAttributesForGeometryFallback(source: Element, target: Element) {
    val skip = setOf(
        "x", "y", "width", "height", "rx", "ry",
        "x1", "y1", "x2", "y2", "cx", "cy", "r", "points",
        "transform", "d"
    )
    ...
    for (i in 0 until attributes.length) {
        ...
        if (name.substringAfter(":").lowercase() !in skip) {
            target.setAttribute(name, attr.nodeValue)
        }
    }
}
